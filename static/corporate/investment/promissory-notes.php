<?include "../../includes/header.php"?>
	<div class="inner deposit">
		<div class="inner__banner">
			<div class="inner__banner-img">
				<div class="inner__banner-img-in" style="background-image: url('/static/theme/images/banners/corporate/investment/promissory-notes.jpg');"></div>
			</div>
			<div class="inner__banner-in grid__inner">
				<div class="inner__banner-block">
					<div class="inner__banner-title">Собственные векселя</div>
					<div class="breadcrumbs">
						<div class="breadcrumbs__item">
							<a href="#" class="breadcrumbs__link">Главная</a>
						</div>
						<div class="breadcrumbs__item">
							<a href="#" class="breadcrumbs__link">Бизнесу</a>
						</div>
						<div class="breadcrumbs__item">
							<a href="#" class="breadcrumbs__link">Размещение свободных средств</a>
						</div>
						<div class="breadcrumbs__item">
							<span>Собственные векселя</span>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="inner__info animation__top"  data-animation>
			<div class="grid__inner">
				<div class="inner__info-block">
					<div class="numerals promissory-notes__block">
						<div class="numerals__block">
							<div class="numerals__item _big">
								<div class="numerals__item-val">8,15%</div>
								<div class="numerals__item-dash"></div>
								<div class="numerals__item-text">
									Максимальная ставка привлечения
								</div>
							</div>
						</div>
					</div>
					<div class="promissory-notes__block is-desktop-block">
						<h4>Операции, проводимые банком с собственными векселями:</h4>
						<div class="deposit-open__methods">
							<div class="deposit-open__method">
								<div class="deposit-open__method-content _only-dash-list">
									<ul class="dash-list">
										<li>Выпуск и оплата (погашение)</li>
										<li>Хранение и досрочный учет</li>
										<li>Новация и размен</li>
										<li>Кредитование под залог векселей</li>
										<li>Подтверждение факта выдачи векселей банком</li>
									</ul>
								</div>
							</div>
						</div>
					</div>
					<div class="base-info promissory-notes__block is-desktop-block">
						<div class="base-info__title">Контакты</div>
						<div class="base-info__subtitle">
							Для проведения операция с векселями РОСГОССТРАх БАНКА Вам необходимо обратиться
							в Управление по работе с собственными ценными бумагами по телефону
						</div>
						<div class="base-info__text">
							<div class="base-info__block">
								<div class="base-info__block-text">
									+7 (495) 925 80 79
								</div>
							</div>
						</div>
					</div>
				</div>
				<div class="inner__info-benefits">
					<div class="inner__info-benefits-title">
						<p>Вексель — это безусловное обязательство Банка выплатить денежные средства в размере номинальной суммы при наступлении срока платежа, указанного в нем.</p>
						<p style="font-weight: 400;">
							РОСГОССТРАХ БАНК предоставляет широкие возможности приобретения и погашения собственных векселей Банка (плательщиком по которым выступает сам Банк), номинированных как в российских рублях, так и в иностранной валюте.
						</p>
						<p style="font-weight: 400;">
							Векселя Банка обращаются на вторичном рынке, котируются в Российской внебиржевой сети как на покупку, так и продажу широким кругом участников вексельного рынка.
						</p>
					</div>
					<div class="promissory-notes__conditions">
						<h3>Условия по операциям с векселями</h3>
						<ul class="b-list">
							<li>Виды валют: рубли, доллары США, евро</li>
							<li>Минимальный номинал: <strong>100 000<span class="rub"></span> / 10 000$/€</strong></li>
							<li>
								По желанию векселеприобретателя при покупке векселей, номинированных в любой мировой валюте,
								можно провести расчеты как в выбранной валюте, так и в рублях РФ по курсу Банка России, действующему на дату приобретения.
								А при погашении векселей — по курсу Банка России, действующему на дату погашения.
							</li>
							<li>Во втором случае покупатель векселей получает валютный инструмент с привлекательной фиксированной доходностью без 
								необходимости открывать валютный счет и конвертировать средства.
							</li>
							<li>Цены векселей: определяются в зависимости от их сроков и утвержденных в Банке процентных ставок.</li>
						</ul>
					</div>
				</div>
				<div class="promissory-notes__block no-desktop-block">
					<h4>Операции, проводимые банком с собственными векселями:</h4>
					<div class="deposit-open__methods">
						<div class="deposit-open__method">
							<div class="deposit-open__method-content _only-dash-list">
								<ul class="dash-list">
									<li>Выпуск и оплата (погашение)</li>
									<li>Хранение и досрочный учет</li>
									<li>Новация и размен</li>
									<li>Кредитование под залог векселей</li>
									<li>Подтверждение факта выдачи векселей банком</li>
								</ul>
							</div>
						</div>
					</div>
				</div>
				<div class="base-info promissory-notes__block no-desktop-block">
					<div class="base-info__title">Контакты</div>
					<div class="base-info__subtitle">
						Для проведения операция с векселями РОСГОССТРАх БАНКА Вам необходимо обратиться
						в Управление по работе с собственными ценными бумагами по телефону
					</div>
					<div class="base-info__text">
						<div class="base-info__block">
							<div class="base-info__block-text">
								+7 (495) 925 80 79
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="b-blocks">
			<div class="b-block _gray animation__top " data-animation>
				<div class="grid__inner">
					<div class="corporate__credits">
						<div class="corporate__credits-block">
							<div class="corporate__credits-block-title">Преимущества вложений средств в векселя:</div>
							<ul class="b-list">
								<li>Выгодное и надежное размещение свободных средств</li>
								<li>Получение фиксированного дохода в установленный срок на гибких и конкурентных условиях</li>
								<li>Приобретение высоколиквидного инструмента для расчетов с контрагентами</li>
								<li>Возможность получения банковских гарантий и кредитов под заклад векселей банка</li>
								<li>Простота оформления</li>
								<li>Отсутствие требования об уведомлении налоговых органов при приобретении векселей</li>
								<li>Высокая скорость проведения операций по выдаче и погашению (в течение)</li>
								<li>Предъявление векселей к погашению в любом филиале РОСГОССТРАХ БАНКА вне зависимости от указанного места платежа в векселе</li>
							</ul>
						</div>
					</div>
				</div>
			</div>
			<div class="b-block animation__top " data-animation>
				<div class="grid__inner">
					<div class="deposit-scale _no-indent is-mob-block">
						<h3>Процентные ставки</h3>
						<div class="deposit-scale__block" data-tabs>
							<div class="deposit-scale__tabs">
								<a href="javascript:void(0);" data-tabs-tab="1"  class="deposit-scale__tab _active">
									<div class="deposit-scale__tab-in valuta"><span class="rub _medium"></span></div>
								</a>
								<a href="javascript:void(0);" data-tabs-tab="2" class="deposit-scale__tab">
									<div class="deposit-scale__tab-in valuta">$</div>
								</a>
								<a href="javascript:void(0);" data-tabs-tab="3" class="deposit-scale__tab">
									<div class="deposit-scale__tab-in valuta">€</div>
								</a>
							</div>
							<div class="deposit-scale__content">
								<div class="deposit-scale__table" data-tabs-content="1">
									<table>
										<tr class="deposit-scale__table-titles">
											<td>СУММА</td>
											<td>от 100 000<br/>до 50 млн</td>
											<td>от 50 млн</td>
										</tr>
										<tr class="deposit-scale__table-row _titles">
											<td class="red">Срок привлечени<br/>(дней)</td>
											<td colspan="2">Максимальная ставка</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td class="red">31-60</td>
											<td>7,65%</td>
											<td>7,90%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">61-90</td>
											<td>7,65%</td>
											<td>7,90%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">91 - 120</td>
											<td>7,90%</td>
											<td>8,15%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">121 - 150</td>
											<td>7,90%</td>
											<td>8,15%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">151 - 180</td>
											<td>7,90%</td>
											<td>8,15%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">181 - 270</td>
											<td>7,90%</td>
											<td>8,15%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">271 - 366</td>
											<td>7,90%</td>
											<td>8,15%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">367 - 545</td>
											<td>7,40%</td>
											<td>7,65%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">546 - 732</td>
											<td>7,40%</td>
											<td>7,65%</td>
										</tr>
									</table>
								</div>
								<div class="deposit-scale__table hide" data-tabs-content="2">
									<table>
										<tr class="deposit-scale__table-titles">
											<td>СУММА</td>
											<td>от 10 000<br/>до 1 000 000</td>
											<td>свыше<br/>1 000 000</td>
										</tr>
										<tr class="deposit-scale__table-row _titles">
											<td class="red">Срок привлечени<br/>(дней)</td>
											<td colspan="2">Максимальная ставка</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">31-60</td>
											<td>0,25%</td>
											<td>0,35%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">61-90</td>
											<td>0,40%</td>
											<td>0,50%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">91 - 120</td>
											<td>0,60%</td>
											<td>0,70%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">121 - 150</td>
											<td>0,70%</td>
											<td>0,80%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">151 - 180</td>
											<td>0,80%</td>
											<td>0,90%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">181 - 270</td>
											<td>0,90%</td>
											<td>1,00%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">271 - 366</td>
											<td>1,70%</td>
											<td>1,80%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">367 - 545</td>
											<td>1,70%</td>
											<td>1,80%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">546 - 732</td>
											<td>1,70%</td>
											<td>1,80%</td>
										</tr>
									</table>
								</div>
								<div class="deposit-scale__table hide" data-tabs-content="3">
									<table>
										<tr class="deposit-scale__table-titles">
											<td>СУММА</td>
											<td>от 10 000<br/>до 1 000 000</td>
											<td>свыше<br/>1 000 000</td>
										</tr>
										<tr class="deposit-scale__table-row _titles">
											<td class="red">Срок привлечени<br/>(дней)</td>
											<td colspan="2">Максимальная ставка</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">31-60</td>
											<td>0,01%</td>
											<td>0,02%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">61-90</td>
											<td>0,01%</td>
											<td>0,02%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">91 - 120</td>
											<td>0,03%</td>
											<td>0,04%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">121 - 150</td>
											<td>0,05%</td>
											<td>0,10%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">151 - 180</td>
											<td>0,10%</td>
											<td>0,20%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">181 - 270</td>
											<td>0,20%</td>
											<td>0,30%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">271 - 366</td>
											<td>0,60%</td>
											<td>0,70%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">367 - 545</td>
											<td>0,60%</td>
											<td>0,70%</td>
										</tr>
										<tr class="deposit-scale__table-row">
											<td  class="red">546 - 732</td>
											<td>0,60%</td>
											<td>0,70%</td>
										</tr>
									</table>
								</div>
							</div>
						</div>
					</div>
					<div class="deposit-scale _no-indent no-mob-block">
						<h3>Процентные ставки</h3>
						<div class="deposit-table__wrap">
							<table class="deposit-table _rowspan-table  _reverse-indents">
								<tr>
									<th colspan="2" class="_left">Срок привлечения (дней)</th>
									<th>31-60</th>
									<th>61-90</th>
									<th>91 - 120</th>
									<th>121 - 150</th>
									<th>151 - 180</th>
									<th>181 - 270</th>
									<th>271 - 366</th>
									<th>367 - 545</th>
									<th>546 - 732</th>
								</tr>
								<tr class="deposit-table__titles">
									<td >ВАЛЮТА ВКЛАДА</td>
									<td>СУММА депозита</td>
									<td colspan="9" class="center">максимальная ставка привлечения, %</td>
								</tr>
								<tr class="deposit-table__row _grey">
									<td class="bold valuta center"><span class="is-desktop-block"></span><span class="rub"></span></td>
									<td >от 100 000 <nobr>до 50 000 000</nobr></td>
									<td class="center bold">7,65</td>
									<td class="center bold">7,65</td>
									<td class="center bold">7,90</td>
									<td class="center bold">7,90</td>
									<td class="center bold">7,90</td>
									<td class="center bold">7,90</td>
									<td class="center bold">7,90</td>
									<td class="center bold">7,40</td>
									<td class="center bold">7,40</td>
								</tr>
								<tr class="deposit-table__row _grey">
									<td></td>
									<td >от 50 000 000</td>
									<td class="center bold">7,90</td>
									<td class="center bold">7,90</td>
									<td class="center bold">8,15</td>
									<td class="center bold">8,15</td>
									<td class="center bold">8,15</td>
									<td class="center bold">8,15</td>
									<td class="center bold">8,15</td>
									<td class="center bold">7,65</td>
									<td class="center bold">7,65</td>
								</tr>
								<tr class="deposit-table__row">
									<td class="bold valuta center">$</td>
									<td >от 10 000 <nobr>до 1 000 000</nobr></td>
									<td class="center bold">0,25</td>
									<td class="center bold">0,40</td>
									<td class="center bold">0,60</td>
									<td class="center bold">0,70</td>
									<td class="center bold">0,80</td>
									<td class="center bold">0,90</td>
									<td class="center bold">1,70</td>
									<td class="center bold">1,70</td>
									<td class="center bold">1,70</td>
								</tr>
								<tr class="deposit-table__row">
									<td></td>
									<td >свыше 1 000 000</td>
									<td class="center bold">0,35</td>
									<td class="center bold">0,50</td>
									<td class="center bold">0,70</td>
									<td class="center bold">0,80</td>
									<td class="center bold">0,90</td>
									<td class="center bold">1,00</td>
									<td class="center bold">1,80</td>
									<td class="center bold">1,80</td>
									<td class="center bold">1,80</td>
								</tr>
								<tr class="deposit-table__row _grey">
									<td class="bold valuta center">€</td>
									<td >от 10 000 <nobr>до 1 000 000</nobr></td>
									<td class="center bold">0,01</td>
									<td class="center bold">0,01</td>
									<td class="center bold">0,03</td>
									<td class="center bold">0,05</td>
									<td class="center bold">0,10</td>
									<td class="center bold">0,20</td>
									<td class="center bold">0,60</td>
									<td class="center bold">0,60</td>
									<td class="center bold">0,60</td>
								</tr>
								<tr class="deposit-table__row _grey">
									<td></td>
									<td >от 3 501 <nobr>до 15 000</nobr></td>
									<td class="center bold">0,02</td>
									<td class="center bold">0,02</td>
									<td class="center bold">0,04</td>
									<td class="center bold">0,10</td>
									<td class="center bold">0,20</td>
									<td class="center bold">0,30</td>
									<td class="center bold">0,70</td>
									<td class="center bold">0,70</td>
									<td class="center bold">0,70</td>
								</tr>
							</table>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
<?include "../../includes/footer.php"?>

