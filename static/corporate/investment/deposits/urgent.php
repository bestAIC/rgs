<?include "../../../includes/header.php"?>
	<div class="inner deposit">
		<div class="inner__banner">
			<div class="inner__banner-img">
				<div class="inner__banner-img-in" style="background-image: url('/static/theme/images/banners/corporate/investment/deposit.jpg');"></div>
			</div>
			<div class="inner__banner-in grid__inner">
				<div class="inner__banner-block">
					<div class="inner__banner-title">Срочный депозит</div>
					<div class="breadcrumbs">
						<div class="breadcrumbs__item">
							<a href="#" class="breadcrumbs__link">Главная</a>
						</div>
						<div class="breadcrumbs__item">
							<a href="#" class="breadcrumbs__link">Бизнесу</a>
						</div>
						<div class="breadcrumbs__item">
							<a href="#" class="breadcrumbs__link">Размещение свободных средств</a>
						</div>
						<div class="breadcrumbs__item">
							<a href="#" class="breadcrumbs__link">Депозиты</a>
						</div>
						<div class="breadcrumbs__item">
							<span>Срочный депозит</span>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="inner__info animation__top _small-indent"  data-animation>
			<div class="grid__inner">
				<div class="inner__info-block">
					<div class="numerals">
						<div class="numerals__block">
							<div class="numerals__item _big">
								<div class="numerals__item-val">8,40%</div>
								<div class="numerals__item-dash"></div>
								<div class="numerals__item-text">
									Максимальная ставка привлечения на депозит в рублях
								</div>
							</div>
						</div>
						<div class="numerals__block">
							<div class="numerals__item _small">
								<div class="numerals__item-val">2,00%</div>
								<div class="numerals__item-dash"></div>
								<div class="numerals__item-text">
									ставка на депозит<br />
									в долларах
								</div>
							</div>
							<div class="numerals__item _small">
								<div class="numerals__item-val">0,75%</div>
								<div class="numerals__item-dash"></div>
								<div class="numerals__item-text">
									ставка на депозит<br />
									в евро
								</div>
							</div>
						</div>
					</div>
				</div>
				<div class="inner__info-benefits">
					<div class="inner__info-benefits-title">
						Срочный депозит с выплатой процентов в конце срока
					</div>
					<ul class="b-list">
						<li>Выплата процентов в конце депозита или ежемесячно</li>
						<li>Возможность льготного досрочного расторжения</li>
					</ul>
					<div class="inner__info-benefits-btn">
						<a href="javascript:void(0);" data-goto=".subscription" class="btn _red">Открыть депозит</a>
					</div>
				</div>
			</div>
		</div>
		<div class="b-blocks">
			<div class="b-block animation__top deposit-info" data-animation>
				<div class="grid__inner">
					<div class="deposit-info__left">
						<div class="deposit-info__block">
							<h3>Общая информация</h3>
							<div class="deposit-conditions">
								<div class="deposit-conditions__item">
									<div class="deposit-conditions__item-block">Валюта депозита</div>
									<div class="deposit-conditions__item-block">рубли, доллары США, евро</div>
								</div>
								<div class="deposit-conditions__item">
									<div class="deposit-conditions__item-block">Пополнение депозита</div>
									<div class="deposit-conditions__item-block">не предусмотрено</div>
								</div>
								<div class="deposit-conditions__item">
									<div class="deposit-conditions__item-block">Процентная ставка</div>
									<div class="deposit-conditions__item-block">6,00%</div>
								</div>
								<div class="deposit-conditions__item">
									<div class="deposit-conditions__item-block">Срок размещения</div>
									<div class="deposit-conditions__item-block">минимальный − 31 день, максимальный − 732 дня</div>
								</div>
								<div class="deposit-conditions__item">
									<div class="deposit-conditions__item-block">Минимальная сумма размещения</div>
									<div class="deposit-conditions__item-block">100 000 рублей /
										10 000 долларов США/евро.</div>
								</div>
								<div class="deposit-conditions__item">
									<div class="deposit-conditions__item-block">Расходование средств</div>
									<div class="deposit-conditions__item-block">не предусмотрено</div>
								</div>
								<div class="deposit-conditions__item">
									<div class="deposit-conditions__item-block">Выплата процентов</div>
									<div class="deposit-conditions__item-block">
										в конце срока депозита. По выбору клиента проценты по депозиту могут выплачиваться ежемесячно
										— в таком случае утвержденная базовая ставка уменьшается на 0,5% годовых
									</div>
								</div>
								<div class="deposit-conditions__item">
									<div class="deposit-conditions__item-block">Капитализация процентов</div>
									<div class="deposit-conditions__item-block">не предусмотрена</div>
								</div>
								<div class="deposit-conditions__item">
									<div class="deposit-conditions__item-block">Пролонгация</div>
									<div class="deposit-conditions__item-block">
										на тех же условиях и при наличии
										заключенного дополнительного
										соглашения к Договору депозита
									</div>
								</div>
								<div class="deposit-conditions__item">
									<div class="deposit-conditions__item-block">Процентная ставка при досрочном расторжении: 	</div>
									<div class="deposit-conditions__item-block">0,50% по депозитам в Рублях, 0,1% по депозитам в Долларах США и Евро</div>
								</div>
							</div>
						</div>
						<div class="deposit-info__block is-mob-block">
							<div class="deposit-scale">
								<h3>Процентные ставки</h3>
								<div class="deposit-scale__block" data-tabs>
									<div class="deposit-scale__tabs">
										<a href="javascript:void(0);" data-tabs-tab="1"  class="deposit-scale__tab _active">
											<div class="deposit-scale__tab-in valuta"><span class="rub _medium"></span></div>
										</a>
										<a href="javascript:void(0);" data-tabs-tab="2" class="deposit-scale__tab">
											<div class="deposit-scale__tab-in valuta">$</div>
										</a>
										<a href="javascript:void(0);" data-tabs-tab="3" class="deposit-scale__tab">
											<div class="deposit-scale__tab-in valuta">€</div>
										</a>
									</div>
									<div class="deposit-scale__content">
										<div class="deposit-scale__table" data-tabs-content="1">
											<table>
												<tr class="deposit-scale__table-titles">
													<td>СУММА<br/>ВКЛАДА</td>
													<td>от 100 000<br/>до 1 млн</td>
													<td>от 1 млн <br/>до 50 млн</td>
													<td>от 50 млн</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td class="red">31-60</td>
													<td>7,65%</td>
													<td>7,90%</td>
													<td>8,15%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">61-90</td>
													<td>7,65%</td>
													<td>7,90%</td>
													<td>8,15%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">91 - 120</td>
													<td>7,90%</td>
													<td>8,15%</td>
													<td>8,40%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">121 - 150</td>
													<td>7,90%</td>
													<td>8,15%</td>
													<td>8,40%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">151 - 180</td>
													<td>7,90%</td>
													<td>8,15%</td>
													<td>8,40%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">181 - 270</td>
													<td>7,90%</td>
													<td>8,15%</td>
													<td>8,40%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">271 - 366</td>
													<td>7,90%</td>
													<td>8,15%</td>
													<td>8,40%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">367 - 545</td>
													<td>7,40%</td>
													<td>7,65%</td>
													<td>7,90%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">546 - 732</td>
													<td>7,40%</td>
													<td>7,65%</td>
													<td>7,90%</td>
												</tr>
											</table>
										</div>
										<div class="deposit-scale__table hide" data-tabs-content="2">
											<table>
												<tr class="deposit-scale__table-titles">
													<td>СУММА<br/>ВКЛАДА</td>
													<td>от 10 000<br/>до 1 000 000</td>
													<td>свыше<br/>1 000 000</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">31-60</td>
													<td>0,30%</td>
													<td>0,40%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">61-90</td>
													<td>0,45%</td>
													<td>0,55%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">91 - 120</td>
													<td>0,50%</td>
													<td>0,75%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">121 - 150</td>
													<td>0,80%</td>
													<td>0,90%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">151 - 180</td>
													<td>0,95%</td>
													<td>1,05%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">181 - 270</td>
													<td>1,15%</td>
													<td>1,25%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">271 - 366</td>
													<td>1,90%</td>
													<td>2,00%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">367 - 545</td>
													<td>1,90%</td>
													<td>2,00%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">546 - 732</td>
													<td>1,90%</td>
													<td>2,00%</td>
												</tr>
											</table>
										</div>
										<div class="deposit-scale__table hide" data-tabs-content="3">
											<table>
												<tr class="deposit-scale__table-titles">
													<td>СУММА<br/>ВКЛАДА</td>
													<td>от 10 000<br/>до 1 000 000</td>
													<td>свыше<br/>1 000 000</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">31-60</td>
													<td>0,01%</td>
													<td>0,02%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">61-90</td>
													<td>0,01%</td>
													<td>0,02%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">91 - 120</td>
													<td>0,03%</td>
													<td>0,04%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">121 - 150</td>
													<td>0,10%</td>
													<td>0,20%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">151 - 180</td>
													<td>0,20%</td>
													<td>0,30%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">181 - 270</td>
													<td>0,30%</td>
													<td>0,40%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">271 - 366</td>
													<td>0,65%</td>
													<td>0,75%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">367 - 545</td>
													<td>0,50%</td>
													<td>0,75%</td>
												</tr>
												<tr class="deposit-scale__table-row">
													<td  class="red">546 - 732</td>
													<td>0,50%</td>
													<td>0,75%</td>
												</tr>
											</table>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div class="deposit-info__right">
						<div class="deposit-open deposit-info__block">
							<h3>Как оформить депозит?</h3>
							<div class="deposit-open__methods">
								<div class="deposit-open__method">
									<div class="deposit-open__method-content">
										<div class="deposit-open__steps _w100">
											<div class="deposit-open__step">
												<div class="deposit-open__step-num">01</div>
												Подготовьте <a href="#">необходимый пакет документов</a> для открытия депозита
											</div>
											<div class="deposit-open__step">
												<div class="deposit-open__step-num">02</div>
												Обратитесь в любое удобное для Вас <a href="#">отделение РОСГОССТРАХ БАНКА, где производится обслуживание юридических лиц</a>
											</div>
											<div class="deposit-open__step">
												<div class="deposit-open__step-num">03</div>
												Подпишите договор  о вкладе с Банком
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
				<div class="grid__inner no-mob-block">
					<div class="deposit-scale">
						<h3>Процентные ставки</h3>
						<div class="deposit-table__wrap">
							<table class="deposit-table _rowspan-table">
								<tr class="deposit-table__titles">
									<td >ВАЛЮТА ВКЛАДА</td>
									<td>СУММА депозита</td>
									<td colspan="9" class="center">максимальная ставка привлечения, %</td>
								</tr>
								<tr>
									<th colspan="2" class="_left">Срок привлечения (дней)</th>
									<th>31-60</th>
									<th>61-90</th>
									<th>91 - 120</th>
									<th>121 - 150</th>
									<th>151 - 180</th>
									<th>181 - 270</th>
									<th>271 - 366</th>
									<th>367 - 545</th>
									<th>546 - 732</th>
								</tr>
								<tr class="deposit-table__row _grey">
									<td class="bold valuta center"><span class="is-desktop-block"></span><span class="rub"></span></td>
									<td >от 100 000 <nobr>до 1 000 000</nobr></td>
									<td class="center bold">7,65</td>
									<td class="center bold">7,65</td>
									<td class="center bold">7,90</td>
									<td class="center bold">7,90</td>
									<td class="center bold">7,90</td>
									<td class="center bold">7,90</td>
									<td class="center bold">7,90</td>
									<td class="center bold">7,40</td>
									<td class="center bold">7,40</td>
								</tr>
								<tr class="deposit-table__row _grey">
									<td></td>
									<td >от 1 000 001 <nobr>до 50 000 000</nobr></td>
									<td class="center bold">7,90</td>
									<td class="center bold">7,90</td>
									<td class="center bold">8,15</td>
									<td class="center bold">8,15</td>
									<td class="center bold">8,15</td>
									<td class="center bold">8,15</td>
									<td class="center bold">8,15</td>
									<td class="center bold">7,65</td>
									<td class="center bold">7,65</td>
								</tr>
								<tr class="deposit-table__row _grey">
									<td></td>
									<td >от 50 000 000</td>
									<td class="center bold">8,15</td>
									<td class="center bold">8,15</td>
									<td class="center bold">8,40</td>
									<td class="center bold">8,40</td>
									<td class="center bold">8,40</td>
									<td class="center bold">8,40</td>
									<td class="center bold">8,40</td>
									<td class="center bold">7,90</td>
									<td class="center bold">7,90</td>
								</tr>
								<tr class="deposit-table__row">
									<td class="bold valuta center">$</td>
									<td >от 10 000 <nobr>до 1 000 000</nobr></td>
									<td class="center bold">0,30</td>
									<td class="center bold">0,45</td>
									<td class="center bold">0,50</td>
									<td class="center bold">0,80</td>
									<td class="center bold">0,95</td>
									<td class="center bold">1,15</td>
									<td class="center bold">1,90</td>
									<td class="center bold">1,90</td>
									<td class="center bold">1,90</td>
								</tr>
								<tr class="deposit-table__row">
									<td></td>
									<td >свыше 1 000 000</td>
									<td class="center bold">0,40</td>
									<td class="center bold">0,55</td>
									<td class="center bold">0,75</td>
									<td class="center bold">0,90</td>
									<td class="center bold">1,05</td>
									<td class="center bold">1,25</td>
									<td class="center bold">2,00</td>
									<td class="center bold">2,00</td>
									<td class="center bold">2,00</td>
								</tr>
								<tr class="deposit-table__row _grey">
									<td class="bold valuta center">€</td>
									<td >от 10 000 <nobr>до 1 000 000</nobr></td>
									<td class="center bold">0,01</td>
									<td class="center bold">0,01</td>
									<td class="center bold">0,03</td>
									<td class="center bold">0,10</td>
									<td class="center bold">0,20</td>
									<td class="center bold">0,30</td>
									<td class="center bold">0,65</td>
									<td class="center bold">0,50</td>
									<td class="center bold">0,50</td>
								<tr class="deposit-table__row _grey">
									<td></td>
									<td >от 3 501 <nobr>до 15 000</nobr></td>
									<td class="center bold">0,02</td>
									<td class="center bold">0,02</td>
									<td class="center bold">0,04</td>
									<td class="center bold">0,20</td>
									<td class="center bold">0,30</td>
									<td class="center bold">0,40</td>
									<td class="center bold">0,75</td>
									<td class="center bold">0,75</td>
									<td class="center bold">0,75</td>
								</tr>
							</table>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="b-blocks _gray">
			<?include "../../../includes/subscription.php"?>
		</div>
	</div>
<?include "../../../includes/footer.php"?>

