<?/*планшет и мобильная версии*/?>
<?/*div class="products swiper-container no-desktop-block"  data-tablet-slider>
	<div class="swiper-wrapper">
		<a href="#" class="product _green swiper-slide" style="background-image: url('/static/theme/images/home/product/1.jpg')">
			<div class="product__inner">
				<div class="product__title">
					Инвестиционное<br />
					страхование жизни
				</div>
				<div class="product__bottom">
					<div class="product__val">100%</div>
					<div class="product__text">
						Гарантия возврата<br />
						вложенных средств
					</div>
				</div>
			</div>
		</a>
		<a href="#" class="product _red swiper-slide" style="background-image: url('/static/theme/images/home/product/2.jpg')">
			<div class="product__inner">
				<div class="product__title">Кредит наличными</div>
				<div class="product__val">под 9%</div>
				<div class="product__bottom">
					<div class="product__text">
						Без комиссий за выдачу<br />
						и обслуживание кредита
					</div>
				</div>
			</div>
		</a>
		<a href="#" class="product _gray swiper-slide" style="background-image: url('/static/theme/images/home/product/3.jpg')">
			<div class="product__inner">
				<div class="product__title">
					Индивидуальный<br />
					инвестиционный<br />
					счет
				</div>
				<div class="product__bottom">
					<div class="product__val">13%</div>
					<div class="product__text">
						Гарантированной<br />
						доходности от государства
					</div>
				</div>
			</div>
		</a>
	</div>
</div*/?>
<?/*десктоп*/?>
<?/*div class="products is-desktop-block" data-animation>
	<div class="grid__inner">
		<a href="#" class="product _green" style="background-image: url('theme/images/home/product/1.jpg')">
			<div class="product__inner">
				<div class="product__title">
					Инвестиционное<br />
					страхование жизни
				</div>
				<div class="product__bottom">
					<div class="product__val">100%</div>
					<div class="product__text">
						Гарантия возврата<br />
						вложенных средств
					</div>
				</div>
			</div>
		</a>
		<a href="#" class="product _brown" style="background-image: url('theme/images/home/product/2.jpg')">
			<div class="product__inner">
				<div class="product__title">Кредит наличными</div>
				<div class="product__val">под 9%</div>
				<div class="product__bottom">
					<div class="product__text">
						Без комиссий за выдачу<br />
						и обслуживание кредита
					</div>
				</div>
			</div>
		</a>
		<a href="#" class="product _gray" style="background-image: url('theme/images/home/product/3.jpg')">
			<div class="product__inner">
				<div class="product__title">
					Индивидуальный<br />
					инвестиционный<br />
					счет
				</div>
				<div class="product__bottom">
					<div class="product__val">13%</div>
					<div class="product__text">
						Гарантированной<br />
						доходности от государства
					</div>
				</div>
			</div>
		</a>
	</div>
</div*/?>
<div class="products" data-animation>
	<div class="products__in">
		<a href="#" class="product _red">
			<div class="product__inner">
				<div class="product__title">
					<span>Инвестиционное<br />
					страхование жизни</span>

				</div>
				<div class="product__bottom">
					<div class="product__val">100%</div>
					<div class="product__text">
						Гарантия возврата<br />
						вложенных средств
					</div>
				</div>
			</div>
			<div class="product__hidden middle" style="background-image: url('/static/theme/images/home/product/1.jpg');">
				<div class="middle__inner">
					<div class="btn _red">Узнать больше</div>
				</div>
			</div>
		</a>
		<a href="#" class="product _brown">
			<div class="product__inner">
				<div class="product__title"><span>Кредит наличными</span></div>
				<div class="product__bottom">
					<div class="product__val">под 9%</div>
					<div class="product__text">
						Без комиссий за выдачу<br />
						и обслуживание кредита
					</div>
				</div>
			</div>
			<div class="product__hidden middle" style="background-image: url('/static/theme/images/home/product/2.jpg');">
				<div class="middle__inner">
					<div class="btn _red">Узнать больше</div>
				</div>
			</div>
		</a>
		<a href="#" class="product _gray">
			<div class="product__inner">
				<div class="product__title">
					<span>Индивидуальный<br />
					инвестиционный счет</span>
				</div>
				<div class="product__bottom">
					<div class="product__val">13%</div>
					<div class="product__text">
						Гарантированной<br />
						доходности от государства
					</div>
				</div>
			</div>
			<div class="product__hidden middle" style="background-image: url('/static/theme/images/home/product/3.jpg');">
				<div class="middle__inner">
					<div class="btn _red">Узнать больше</div>
				</div>
			</div>
		</a>
	</div>
</div>
