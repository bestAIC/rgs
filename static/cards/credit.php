<?include "../includes/header.php"?>
	<div class="inner deposit">
		<div class="inner__banner">
			<div class="inner__banner-img _blur">
				<div class="inner__banner-img-in" style="background-image: url('/static/theme/images/banners/card/Card.jpg');"></div>
			</div>
			<div class="inner__banner-in grid__inner">
				<div class="b-card _blue no-mob-block"></div>
				<div class="inner__banner-block">
					<div class="inner__banner-title">Кредитные карты для сотрудников группы&nbsp;РОСГОССТРАХ</div>
					<div class="breadcrumbs">
						<div class="breadcrumbs__item">
							<a href="#" class="breadcrumbs__link">Частным клиентам</a>
						</div>
						<div class="breadcrumbs__item">
							<a href="#" class="breadcrumbs__link">Выбрать карту</a>
						</div>
						<div class="breadcrumbs__item">
							<span>Для сотрудников группы Росгосстрах</span>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="inner__info animation__top"  data-animation>
			<div class="grid__inner">
				<div class="inner__info-block">
					<div class="numerals">
						<div class="numerals__block">
							<div class="numerals__item _big">
								<div class="numerals__item-val">23%</div>
								<div class="numerals__item-dash"></div>
								<div class="numerals__item-text">
									ставка по кредиту
								</div>
							</div>
						</div>
						<div class="numerals__block">
							<div class="numerals__item _small">
								<div class="numerals__item-val">1 <span>год</span></div>
								<div class="numerals__item-dash"></div>
								<div class="numerals__item-text">
									бесплатного
									обслуживания карты
								</div>
							</div>
							<div class="numerals__item _small">
								<div class="numerals__item-val"><span>до</span> 50 <span>дней</span></div>
								<div class="numerals__item-dash"></div>
								<div class="numerals__item-text">
									льготный период
									кредитования
								</div>
							</div>
						</div>
					</div>
				</div>
				<div class="inner__info-benefits">
					<div class="inner__info-benefits-title">
						Преимущества кредитной карты
					</div>
					<ul class="b-list">
						<li>Кредитный лимит до 300 000 рублей</li>
						<li>Возобновляемая кредитная линия</li>
						<li>Погашение задолженности минимальными платежами —
							10% от суммы основного долга + начисленные проценты
						</li>
						<li>Карта <a href="#">премиальной категории<</a>/li>
						<li>Скидки и привилегии в компаниях-партнерах Банка</li>
						<li>Бесплатный Интернет-Банк и SMS-информирование</li>
					</ul>
					<div class="inner__info-benefits-btn">
						<a href="#" class="btn _red">Оформить карту</a>
					</div>
				</div>
			</div>
		</div>
		<div class="b-blocks _gray">
			<?include "../includes/subscription.php"?>
			<?include "../includes/offices.php"?>
			<div class="b-block animation__top" data-animation>
				<div class="grid__inner">
					<?include "../includes/deposit-doc.php"?>
					<div class="files _two-cols">
						<a href="#" class="file">
							<div class="file__inner">
								<div class="file__name">Тарифы по оплате услуг, предоставляемых физическим лицам</div>
								<div class="file__props">PDF, 450.96KB</div>
							</div>
						</a>
						<a href="#" class="file">
							<div class="file__inner">
								<div class="file__name">Вопросник для физических лиц</div>
								<div class="file__props">DOC, 70.42KB</div>
							</div>
						</a>
					</div>
				</div>
			</div>
			<?include "../includes/questions.php"?>
			<?include "../includes/news-gall.php"?>
		</div>
	</div>
<?include "../includes/footer.php"?>

