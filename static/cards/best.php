<?include "../includes/header.php"?>
	<div class="inner deposit">
		<div class="inner__banner">
			<div class="inner__banner-img _blur">
				<div class="inner__banner-img-in" style="background-image: url('/static/theme/images/banners/card/Otlichnaya.jpg');"></div>
			</div>
			<div class="inner__banner-in grid__inner">
				<div class="b-card _blue"></div>
				<div class="inner__banner-block">
					<div class="inner__banner-title">Отличная карта</div>
					<div class="breadcrumbs">
						<div class="breadcrumbs__item">
							<a href="#" class="breadcrumbs__link">Частным клиентам</a>
						</div>
						<div class="breadcrumbs__item">
							<a href="#" class="breadcrumbs__link">Выбрать карту</a>
						</div>
						<div class="breadcrumbs__item">
							<span>Отличная карта</span>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="inner__info animation__top"  data-animation>
			<div class="grid__inner">
				<div class="inner__info-block">
					<div class="numerals">
						<div class="numerals__block">
							<div class="numerals__item _big">
								<div class="numerals__item-val">5,00%</div>
								<div class="numerals__item-dash"></div>
								<div class="numerals__item-text">
									на остаток средств на карте
								</div>
							</div>
						</div>
						<div class="numerals__block">
							<div class="numerals__item _small">
								<div class="numerals__item-val">5%</div>
								<div class="numerals__item-dash"></div>
								<div class="numerals__item-text">
									cashback при оплате
									в&nbsp;продовольственных
									магазинах
								</div>
							</div>
							<div class="numerals__item _small">
								<div class="numerals__item-val">5 <span>минут</span></div>
								<div class="numerals__item-dash"></div>
								<div class="numerals__item-text">
									на оформление
								</div>
							</div>
						</div>
					</div>
				</div>
				<div class="inner__info-benefits">
					<div class="inner__info-benefits-title">
						Преимущества Отличной карты
					</div>
					<ul class="b-list">
						<li>5 минут на оформление</li>
						<li>5% годовых на среднемесячный остаток средств по карте</li>
						<li>5% CashBack при оплате Отличной картой в продовольственных магазинах</li>
						<li>1% CashBack на все операции при оплате картой по всему миру</li>
						<li><a href="#">Интернет-Банк</a> и <a href="#">Мобильный Банк</a> - бесплатный и безопасный доступ к управлению Вашими счетами 24 часа 7 дней в неделю</li>
					</ul>
					<div class="inner__info-benefits-btn">
						<a href="#" class="btn _red">Оформить карту</a>
					</div>
				</div>
			</div>
		</div>
		<div class="b-blocks _gray">
			<?include "../includes/subscription.php"?>
			<?include "../includes/offices.php"?>
			<div class="b-block animation__top" data-animation>
				<div class="grid__inner">
					<?include "../includes/deposit-doc.php"?>
					<div class="files _two-cols">
						<a href="#" class="file">
							<div class="file__inner">
								<div class="file__name">Тарифы по оплате услуг, предоставляемых физическим лицам</div>
								<div class="file__props">PDF, 450.96KB</div>
							</div>
						</a>
						<a href="#" class="file">
							<div class="file__inner">
								<div class="file__name">Вопросник для физических лиц</div>
								<div class="file__props">DOC, 70.42KB</div>
							</div>
						</a>
					</div>
				</div>
			</div>
			<?include "../includes/questions.php"?>
			<?include "../includes/news-gall.php"?>
		</div>
	</div>
<?include "../includes/footer.php"?>

